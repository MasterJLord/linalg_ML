...
def BackPropagation(inputs : np.array, label : float, activationFunctions : np.array, weights : np.array, learningRate : float, lossFunction : str = "L2"):
    ...
    if (lossFunction == "L2"):
        ...
        outputLosses.append(outputMultiple * weights[len(weights)-1])
    else:
    ...
    for layerIndex in range(len(weights)-1, 0, -1): 
        ...
        layerOutputLosses = np.zeros(len(weights[layerIndex-1]))
        ...
        layerOutputLosses[0] = 0 # TODO : Make sure the way I'm handling biases works
        outputLosses.insert(0, layerOutputLosses)
    ...
    for layerIndex in range(len(weights)):
        for neuronNum in range(len(weights[layerIndex])):
            ...
            for weight in range(len(weights[layerIndex][neuronNum])):
                weights[layerIndex][neuronNum][weight] -= activationFunc((inputs if layerIndex == 0 else allOutputs[layerIndex-1])[weight]) * lossMultiplier